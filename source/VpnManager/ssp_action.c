...
PCOMPONENT_COMMON_VPN_MANAGER            g_pComponentCommonVpnManager      = NULL;
...
#define  DATAMODEL_XML_FILE              "/usr/rdk/vpnmanager/RdkVpnManager.xml"
...
ANSC_STATUS ssp_create()
{
    ...
    if(!g_pComponentCommonVpnManager)
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit(g_pComponentCommonVpnManager);

    g_pComponentCommonVpnManager->Name    = AnscCloneString(RDK_COMPONENT_NAME_VPN_MANAGER);
    g_pComponentCommonVpnManager->Version = 1;
    g_pComponentCommonVpnManager->Author  = AnscCloneString("Pentagon");
    ...
    if (!pDslhLcbIf)
    {
        ...
        else
        {
            ...
            pDslhLcbIf->InitLibrary              = VpnDmlInit;
        }
    }
    ...
    )
{
    ...
    g_pComponentCommonVpnManager->Health = RDK_COMMON_COMPONENT_HEALTH_Yellow;
    ...
    returnStatus =
        ...
        (
         ...
         RDK_COMPONENT_NAME_VPN_MANAGER,            /* Component Name    */
         RDK_COMPONENT_VERSION_VPN_MANAGER,         /* Component Version */
         RDK_COMPONENT_PATH_VPN_MANAGER,            /* Component Path    */
    ...
    )
{
    ...
    if(  g_pComponentCommonVpnManager == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    {
        sprintf(CrName, "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        sprintf(CpName, "%s%s", g_Subsystem, RDK_COMPONENT_NAME_VPN_MANAGER);
    }
    else
    {
        sprintf(CrName, "%s", CCSP_DBUS_INTERFACE_CR);
        sprintf(CpName, "%s", RDK_COMPONENT_NAME_VPN_MANAGER);
    }
...
{
    return  g_pComponentCommonVpnManager->Name;
}
...
{
    return  g_pComponentCommonVpnManager->Version;
}
...
{
    return  g_pComponentCommonVpnManager->Author;
}
...
{
    return  g_pComponentCommonVpnManager->Health;
}
...
{
    return  g_pComponentCommonVpnManager->State;
}
...
{
    return  g_pComponentCommonVpnManager->LogEnable;
}
...
{
    if(g_pComponentCommonVpnManager->LogEnable == bEnabled) 
    {
        return ANSC_STATUS_SUCCESS;
    }

    g_pComponentCommonVpnManager->LogEnable = bEnabled;
        
    if(bEnabled) 
    {
        g_iTraceLevel = (INT) g_pComponentCommonVpnManager->LogLevel;
    }
    else 
    {
...
{
    return  g_pComponentCommonVpnManager->LogLevel;
}
...
{
    if(g_pComponentCommonVpnManager->LogLevel == LogLevel) 
    {
        return ANSC_STATUS_SUCCESS;
    }

    g_pComponentCommonVpnManager->LogLevel = LogLevel;

    if(g_pComponentCommonVpnManager->LogEnable) 
    {
        g_iTraceLevel = (INT) g_pComponentCommonVpnManager->LogLevel;
    }
...
{
    return  g_pComponentCommonVpnManager->MemMinUsage;
}
    ...
    )
{
    ...
    size = AnscGetComponentMemorySize(RDK_COMPONENT_NAME_VPN_MANAGER);
    if (size == -1 )
        size = 0;
    ...
    )
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponentCommonVpnManager->LogLevel); */
    return returnStatus;
}
